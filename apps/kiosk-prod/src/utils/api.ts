...
// 旧版 Safari/iOS 无 crypto.randomUUID，用 getRandomValues 实现 UUID v4
function generateUUID(): string {
  const c = (crypto as any).getRandomValues ? crypto : (globalThis as any).msCrypto
...
function getOrCreateUUID(): string {
  ...
  if (!uuid) {
    uuid = typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : generateUUID()
