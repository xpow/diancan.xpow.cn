  ...
  if (props.group.type === 'multi') {
    ...
    if (idx > -1) {
      ...
      if (props.group.name === '口味') {
        ...
        arr = arr.filter((v) => v !== '不加料'); arr.push(label)
      }
    }
...
.selector-chip { display: flex; align-items: center; gap: 1px; padding: 4px 10px; border-radius: var(--radius-xl); border: 1px solid var(--outline-variant); background: transparent; color: var(--on-surface); font-family: var(--font-display); font-size: var(--text-label-sm); font-weight: 800; cursor: pointer; transition: all var(--transition-fast); }
