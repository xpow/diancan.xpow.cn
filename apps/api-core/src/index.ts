...
/* ===== 加单分组 ===== */
app.post('/api/orders/:orderNo/group', orderLimiter, authMiddleware, async (req, res) => {
  const { orderNo } = req.params
  ...
  // 共享 groupId：如果目标已有 groupId 则沿用，否则用目标的 orderNo 生成
...
/* ===== 取消分组 ===== */
app.post('/api/orders/:orderNo/ungroup', orderLimiter, authMiddleware, async (req, res) => {
  const { orderNo } = req.params
