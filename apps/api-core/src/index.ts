...
/* ===== 合并订单（加单） ===== */
app.post('/api/orders/:orderNo/merge', orderLimiter, authMiddleware, async (req, res) => {
  const { orderNo } = req.params
  ...
  const source = await prisma.order.findUnique({ where: { orderNo }, include: { items: true } })
  if (!source) return res.status(404).json({ message: '源订单不存在' })

  const target = await prisma.order.findUnique({ where: { orderNo: targetOrderNo }, include: { items: true } })
  ...
  try {
    const merged = await prisma.$transaction(async (tx) => {
      ...
      for (const item of allItems) {
        ...
        originalAmount += item.unitPrice * item.quantity / portionFactor
      ...
      for (const promo of fullReductionPromos) {
        const rules = JSON.parse(promo.rules)
        if (payableAmount >= (rules.threshold ?? 0) && (rules.discount ?? 0) > 0) {
          fullReductionDiscount = rules.discount
          break
        }
      }
